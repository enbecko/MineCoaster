...
public class EntityRenderer2 extends EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public EntityRenderer2(Minecraft p_i45076_1_, IResourceManager p_i45076_2_)
    {
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                catch (Throwable throwable)
                {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen name", new Callable()
                    {
                        ...
                        public String call()
                        {
                            return EntityRenderer2.this.mc.currentScreen.getClass().getCanonicalName();
                        }
                    });
    ...
    public void setAngles(float p_70082_1_, float p_70082_2_)
    {
        ...
        mc.thePlayer.rotationYaw = (float)((double)mc.thePlayer.rotationYaw + (double)p_70082_1_ * 0.15D);
        mc.thePlayer.rotationPitch = (float)((double)mc.thePlayer.rotationPitch - (double)p_70082_2_ * 0.15D);
        //mc.thePlayer.rotationPitch = MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationPitch);
        if(mc.thePlayer.rotationPitch > 360 || mc.thePlayer.rotationPitch < -360)
        	mc.thePlayer.rotationPitch = 0;
        if(mc.thePlayer.ridingEntity instanceof EntityTrain)
        {
        	/**
        	EntityTrain tr = ((EntityTrain)mc.thePlayer.ridingEntity);
        	int upper = -360, downer = 360;
        	if(tr.act != null && tr.act instanceof EntityTrackUp)
        	{
	            if(!tr.act.down)
	            {
	            	upper = -180;
	            ...
	            {
	            	if(mc.thePlayer.rotationPitch < 180 && mc.thePlayer.rotationPitch > 0)
	            		mc.thePlayer.rotationPitch = -180-(180-mc.thePlayer.rotationPitch);
	            	upper = 0;
	            	downer = 180;
	            }
        	}
        	if(tr.act != null && tr.act instanceof EntityTrackHorTop)
        	{
	            if(!tr.act.down)
	            {
	            	upper = -225;
        	...
        	if(tr.act != null && (tr.act instanceof EntityTrack225Top || tr.act instanceof EntityTrack225_2Top && tr.act.alignment > 3))
        	{
	            if(!tr.act.down)
	            {
	            	upper = -202;
        	...
        	if(tr.act != null && tr.act instanceof EntityTrackBaseTop)
        	{
	            upper = -270;
	            downer = -90;
        	}
        	System.out.println(mc.thePlayer.rotationPitch);
	        if (mc.thePlayer.rotationPitch < upper)
	        {
	           	mc.thePlayer.rotationPitch = upper;
	        }
	
	        if (mc.thePlayer.rotationPitch > downer)
	        {
	         	mc.thePlayer.rotationPitch = downer;
	        }*/
        }else
        {     
        ...
        {
	        if(mc.thePlayer.rotationYaw > mc.thePlayer.prevRotationYaw)
	        	mc.thePlayer.rotationYaw -= 2*(mc.thePlayer.rotationYaw - mc.thePlayer.prevRotationYaw);
	        else
	        	mc.thePlayer.rotationYaw -= 2*(mc.thePlayer.rotationYaw - mc.thePlayer.prevRotationYaw);
        }
        
        mc.thePlayer.prevRotationPitch += mc.thePlayer.rotationPitch - f2;
        mc.thePlayer.prevRotationYaw += mc.thePlayer.rotationYaw - f3;
    }
